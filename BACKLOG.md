...
- **Triage GA** (`.github/workflows/triage.yml` + `skill/triage/`) — fires on `on: issues: [opened, edited]`. Runs on ubuntu-latest (Claude API). Loads skill/triage. Actions per new issue: categorize (type/priority labels), dedupe against open issues, consolidate 3+ clustered issues, route (label 'ready' or 'needs-info' or 'translate' etc.). Closes the loop from Codex findings → filed issues → routed to Builder or human. Third leg of the agent trio (Builder + Review + Triage); all three same Claude class, different trigger + skill.
