...
state graph rather than imperative functions, so the two graphs stay aligned. Where a
native SwiftUI construct maps cleanly, Swami uses it. Where Origami's semantics need a
helper, Swami ships a helper named after the Origami patch and shaped like it.
