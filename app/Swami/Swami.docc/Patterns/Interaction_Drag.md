...
Purple (`#DD70DF`) and hides application and status-bar chrome. A lighter
lavender (`#E5A6E6`) 315×607 canvas sits 30 points inside every viewport edge
with a 20-point corner radius. The centered 120×120 white layer is the drag
target. These bounds, radii, spacing, and rendered colors are measured from the
authenticated 750×1334 runner reference rather than inferred from simulator
chrome or an unrelated library definition.
