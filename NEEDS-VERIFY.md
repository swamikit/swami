...
- **`.github/ISSUE_TEMPLATE/infra-blocker.md`** — structured evidence template for when a Builder or Review GA hits a runner-level failure (Origami install broken, sim boot fails, ImageMagick not available, etc.). Fields: what step failed, full log excerpt, environment (macos-15 image, Xcode version, Origami version), what was tried, suspected cause. **Template auto-applies `label: infra-blocker`** (via `labels: [infra-blocker]` in the template frontmatter) so downstream queries are label-based, not type-based (`--type` was undecided in `gh` versions across the fleet; labels are portable). Its own tiny PR.
